          ...
          <div className='space-y-4'>
            {orderedSessions.slice(0, 3).map((session: any) => (
              <Link key={session._id.toString()} href={`/campaigns/${session.campaignId}/sessions/${session._id}`}>
                <div key={session._id} className='flex items-start space-x-3 p-3 bg-secondary/50 rounded-lg mb-6'>
                  <CalendarDays className='h-5 w-5 text-muted-foreground mt-0.5' />
                  <div>
                    <p className='font-medium text-foreground'>
                      Session {session.sessionNumber} {format(new Date(session.date), 'PPP')}
                    </p>
                    <p className='text-sm text-muted-background'>{session.summary}</p>
                  </div>
                </div>
              </Link>
            ))}
          ...
          <div className='space-y-4'>
            <div className='flex items-center justify-between p-3 bg-secondary/50 rounded-lg'>
              <div className='flex items-center space-x-3'>
                ...
                <div>
                  <p className='font-medium text-foreground'>Player Level</p>
                  <p className='text-sm text-muted-foreground'>Current: {campaign.level.current}</p>
                </div>
              </div>
              <p className='text-sm text-muted-foreground'>Started at level {campaign.level.start}</p>
            </div>

            {
              <div className='flex items-center justify-between p-3 bg-secondary/50 rounded-lg'>
                <div className='flex items-center space-x-3'>
                  ...
                  <div>
                    <p className='font-medium text-foreground'>Quests Completed</p>
                    <p className='text-sm text-muted-foreground'>{campaign.quests.completedQuests.length} quests</p>
                  </div>
                </div>
              </div>
            }
